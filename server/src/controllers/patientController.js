...
export const register = tryCatchFn(async (req, res, next) => {
  const { id: userId } = req.user; //we destructure it Now userId contains the actual User._id (the MongoDB ObjectId string).
...
export const updatePatient = tryCatchFn(async (req, res, next) => {
  const { id: patientId } = req.params;  //It grabs the id property from req.params. Then renames it locally to patientId.
