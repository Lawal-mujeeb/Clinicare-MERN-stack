...
import { useCallback, useRef } from "react"; //we are usimg gthe useref to place an id on the input field, help us traget it
...
export default function UploadImage() {
  ...
  //fileRef → a hidden <input type="file" /> reference, so you can trigger it programmatically (when user clicks “Change image”).
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.status === 200) {
        ...
        queryClient.invalidateQueries({ queryKey: ["auth_user"] }); //👉 “The data you have cached for ["auth_user"] is now outdated. Please refetch it from the server.”
      }
    },
    onError: (error) => {
  ...
  //👉 This opens the hidden file input when the user clicks “Change image”.
  const handleImageClick = () => {
    if (fileRef.current) {
      fileRef.current.value = ""; // reset input field
        ...
        <div className="avatar avatar-placeholder relative">
          <div className="w-20 rounded-full bg-gray-300 text-gray-600">
            {user?.avatar || selectedFile ? (
              <img
                src={selectedFile ? selectedFile : user?.avatar} //If the user has already uploaded an avatar, display it.If they just selected a new file, show its preview.Otherwise, show their initials.
          ...
          {/* //If they picked a file, they can remove/reset it. */}
          {selectedFile && (
            <button
              type="button"
